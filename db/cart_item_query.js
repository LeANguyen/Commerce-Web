  ...
  pool.query(
    `INSERT INTO cart_item (item_id, cart_id, quantity) 
                VALUES ($1, (SELECT id 
                            ...
                            FROM (SELECT * FROM cart WHERE client_id = $2) AS X)), $3)`,
    [item_id, client_id, quantity],
    (error, results) => {
      if (error) {
        throw error;
      }
...
const getItemFromCurrentCartByItemID = (request, response) => {
  ...
  pool.query(
    `SELECT * FROM cart_item WHERE item_id=$1 AND cart_id=(SELECT MAX(id) FROM (SELECT * FROM cart WHERE client_id = $2) AS X)`,
    [item_id, client_id],
    (error, results) => {
      if (error) {
        throw error;
...
const getAllItemFromCurrentCart = (request, response) => {
  ...
  pool.query(
    `SELECT cart_item.item_id, item.item_name, item.category, item.origin, item.price, cart_item.cart_id, cart_item.quantity 
                ...
                WHERE cart_id=(SELECT MAX(id) FROM (SELECT * FROM cart WHERE client_id = $1) AS X)`,
    [client_id],
    (error, results) => {
      if (error) {
        throw error;
...
const deleteItemFromCurrentCart = (request, response) => {
  ...
  pool.query(
    `DELETE FROM cart_item WHERE item_id = $1 AND cart_id = (SELECT MAX(id) 
    FROM (SELECT * FROM cart WHERE client_id = $2) AS X)`,
    [item_id, client_id],
    (error, results) => {
      if (error) {
        throw error;
